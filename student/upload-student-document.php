        ...
        <div class="row justify-content-center">
            <div class="col-lg-8">
                ...
                <div class="d-flex justify-content-end mb-3">
                    <a href="student-dashboard?reg=<?= urlencode($reg_no) ?>" class="btn btn-outline-secondary">
                ...
                <div class="card upload-card">
                    <div class="card-header bg-primary text-white">
                        <h5 class="mb-0">
                            <i class="bi bi-cloud-upload me-2"></i>Upload Documents for
                            <?= htmlspecialchars($student['student_name']) ?>
                        </h5>
                        <small>Reg No: <?= htmlspecialchars($reg_no) ?></small>
                    </div>
                    <div class="card-body p-4">
                        <form action="process-document-upload.php" method="POST" enctype="multipart/form-data"
                            ...
                            <div class="row g-3">
                                ...
                                foreach ($documents as $doc) {
                                    $existingTypes[] = $doc['photo_type'];   // e.g. AADHAR, PAN, SIGNATURE, ...
                                }
                                ?>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select class="form-select form-select-lg" name="photo_type" id="photoType"
                                        required>
                                        <option value="">-- Select Document Type --</option>

                                        <option value="AADHAR" <?= in_array('AADHAR', $existingTypes, true) ? 'disabled' : '' ?>>
                                            Aadhar Card
                                        </option>

                                        <option value="SIGNATURE" <?= in_array('SIGNATURE', $existingTypes, true) ? 'disabled' : '' ?>>
                                            Signature
                                        </option>
                                        
                                        <option value="" disabled>──────────</option>
                                        <option value="CASTE_CERT" <?= in_array('CASTE_CERT', $existingTypes, true) ? 'disabled' : '' ?>>
                                            Caste Certificate
                                        </option>
                                        <option value="DOMICILE_CER" <?= in_array('DOMICILE_CER', $existingTypes, true) ? 'disabled' : '' ?>>
                                            Residence Certificate
                                        </option>
                                        <option value="INCOME_CERT" <?= in_array('INCOME_CERT', $existingTypes, true) ? 'disabled' : '' ?>>
                                            Income Certificate
                                        </option>
                                        <option value="" disabled>──────────</option>
                                        <option value="OTHERS" <?= in_array('OTHERS', $existingTypes, true) ? 'disabled' : '' ?>>
                                            Others
                                        </option>
                                    </select>
                                </div>
                                ...
                                <div class="col-md-6">
                                    <label class="form-label fw-bold">
                                        <i class="bi bi-file-earmark me-1"></i>Select File <span
                                            class="text-danger">*</span>
                                    </label>
                                    <input type="file" class="form-control form-control-lg" name="document"
                                        id="documentFile" accept=".pdf,.jpg,.jpeg,.png,.doc,.docx" required>
                                ...
                                <div class="document-item d-flex justify-content-between align-items-center">
                                    <div>
                                        ...
                                        <small class="text-muted">
                                            <i
                                                class="bi bi-calendar me-1"></i><?= date('d-M-Y H:i', strtotime($doc['upload_date'])) ?>
