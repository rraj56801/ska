...
<div class="container mt-5 mb-5">
    ...
    <a href="student-dashboard?reg=<?= urlencode($reg_no) ?>"
...
?>
    ...
    <select class="form-select form-select-lg" name="photo_type" id="photoType" required>
        ...
        <option value="AADHAR"
            <?= in_array('AADHAR', $existingTypes, true) ? 'disabled' : '' ?>>
            Aadhar Card
        </option>

        <option value="PAN"
            <?= in_array('PAN', $existingTypes, true) ? 'disabled' : '' ?>>
            PAN Card
        </option>

        <option value="VOTER_ID"
            <?= in_array('VOTER_ID', $existingTypes, true) ? 'disabled' : '' ?>>
            Voter ID
        </option>

        <option value="SIGNATURE"
            <?= in_array('SIGNATURE', $existingTypes, true) ? 'disabled' : '' ?>>
            Signature
        </option>

        <option value="OTHERS"
            <?= in_array('OTHERS', $existingTypes, true) ? 'disabled' : '' ?>>
            Others
        </option>
    </select>
</div>
