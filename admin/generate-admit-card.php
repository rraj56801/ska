...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['confirm'])) {
        // Generate Admit Card
        $stmt = $pdo->prepare("UPDATE students SET admit_card_gen = 'Yes' WHERE reg_no = ?");
        $stmt->execute([$reg_no]);
    ...
    if (isset($_POST['disable'])) {
        // Disable Admit Card
        $stmt = $pdo->prepare("UPDATE students SET admit_card_gen = 'No' WHERE reg_no = ?");
        $stmt->execute([$reg_no]);
...
<head>
    ...
    <title>Admit Card Management - <?= htmlspecialchars($student['student_name']) ?></title>
    ...
    <style>
        * {
        ...
        .status-badge {
            ...
            right: 20px;
            background:
                <?= $student['admit_card_gen'] == 'Yes' ? '#10b981' : '#3b82f6' ?>
            ;
            color: white;
            padding: 10px 20px;
            border-radius: 25px;
        ...
        .status-icon {
            ...
            margin-bottom: 20px;
            color:
                <?= $student['admit_card_gen'] == 'Yes' ? '#10b981' : '#6b7280' ?>
            ;
        }
...
<body>
    <div class="main-container">
        ...
        <div class="page-header">
            <h1 class="page-title">Admit Card Management</h1>
            <p class="page-subtitle">Generate or disable student admit cards</p>
        </div>
                ...
                <div class="status-container">
                    <div class="status-card position-relative">
                        <div class="status-badge">
                            <?= $student['admit_card_gen'] == 'Yes' ? 'ACTIVE' : 'PENDING' ?>
                        </div>

                        <?php if ($student['admit_card_gen'] == 'Yes'): ?>
                            <i class="bi bi-door-open status-icon"></i>
                            <div class="status-title">Admit Card Active</div>
                            <div class="status-subtitle">Admit card generated successfully</div>
                        <?php else: ?>
                            <i class="bi bi-door-closed status-icon"></i>
                            <div class="status-title">Admit Card Pending</div>
            ...
            <div class="confirm-section">
                <?php if ($student['admit_card_gen'] != 'Yes'): ?>
                    ...
                    <p class="confirm-subtitle">
                        Confirm admit card generation for this student.
                    </p>
                    ...
                    <form method="POST">
                        ...
                        <div class="form-check d-flex align-items-center p-4 border rounded-3 mb-4"
                            ...
                            <input class="form-check-input me-4" type="checkbox" name="generate_admit" id="confirm_yes"
                                value="yes" required>
                            <label class="form-check-label flex-grow-1 mb-0 p-3" for="confirm_yes">
                                ...
                                Yes, Generate Admit Card for this student
                            </label>
                        </div>

                        <div class="btn-group">
                            <button type="submit" class="btn btn-confirm" id="generateBtn" disabled>
                                <i class="bi bi-door-open me-2"></i>Generate Admit Card
                            </button>
                ...
                <?php else: ?>
                    <div class="disable-section">
                        ...
                        <h2 style="color: #10b981;">Admit Card Active</h2>
                        <p class="fs-5 mb-4">Admit card has been generated for this student.</p>

                        <form method="POST" style="display: inline;">
                            <input type="hidden" name="disable" value="1">
                            <div class="btn-group">
                                <button type="submit" class="btn btn-disable">
                                    <i class="bi bi-ban me-2"></i>Disable Admit Card
                                </button>
