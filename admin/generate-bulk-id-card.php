...
// ---------- FILTERS ----------
$where = "WHERE (s.id_card_generated = 'No' OR s.id_card_generated IS NULL)";
$params = [];
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!empty($selected_reg_nos)) {
        ...
        $stmt = $pdo->prepare("UPDATE students SET id_card_generated = 'Yes' WHERE reg_no IN ($placeholders)");
        $stmt->execute($selected_reg_nos);

        $count = count($selected_reg_nos);
        $message = "Successfully generated ID cards for {$count} student(s)!";
...
<head>
    ...
    <title>Bulk Generate ID Cards</title>
            ...
            <div class="col-lg-11">
                <div class="card shadow-lg border-0 rounded-4">
                    <div class="card-header bg-gradient bg-primary text-white text-center py-5">
                        <h2 class="mb-0">
                            <i class="bi bi-card-text me-2"></i>Bulk Generate ID Cards
                        </h2>
                        <p class="mb-0 opacity-90 mt-2">Select students → Generate ID cards in bulk → Print individually
                        </p>
                    </div>
                        ...
                        <div class="row mb-5">
                            <div class="col-md-4">
                                <div class="card stats-card text-center p-4 rounded-3 h-100">
                                    ...
                                    <small>Pending ID Cards (Filtered)</small>
                                </div>
                            </div>
                            <div class="col-md-4">
                        ...
                        <div class="row g-4 mb-4">
                            <div class="col-12">
                                <div class="select-zone text-center p-5 position-relative">
                                    ...
                                    <p class="text-muted mb-0">Check boxes → Generate ID status → Print individual
                                        cards</p>
                                    <?php if (empty($students)): ?>
                                        <div class="mt-4 p-4 bg-light rounded-3">
                                            <i class="bi bi-check-circle fs-3 text-success mb-3"></i>
                                            <h6 class="text-success">All ID Cards Generated!</h6>
                                        </div>
                                    <?php endif; ?>
                                </div>
                            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function updateGenerateButton() {
                ...
                if (generateBtn) {
                    generateBtn.innerHTML = `Generate Selected ID Cards (${checkedCount})`;
