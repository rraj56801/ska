...
// ---------- FILTERS ----------
$filter_reg_no = $_GET['reg_no'] ?? '';
$filter_course = $_GET['course'] ?? '';
$filter_center = $_GET['center'] ?? '';

$where = "WHERE (s.certificate_gen = 'No' OR s.certificate_gen IS NULL)";
$params = [];

// Reg No (partial)
if ($filter_reg_no !== '') {
    $where .= " AND s.reg_no LIKE :reg_no";
    $params[':reg_no'] = '%' . $filter_reg_no . '%';
}

// Course (exact by code)
if ($filter_course !== '') {
    $where .= " AND s.course_code = :course";
    $params[':course'] = $filter_course;
}

// Center (exact by code)
if ($filter_center !== '') {
    $where .= " AND s.study_center_code = :center";
    $params[':center'] = $filter_center;
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!empty($selected_reg_nos)) {
        ...
        $stmt = $pdo->prepare("UPDATE students SET certificate_gen = 'Yes', certificate_gen_date = NOW()  WHERE reg_no IN ($placeholders)");
        $stmt->execute($selected_reg_nos);

        $count = count($selected_reg_nos);
        $message = "Successfully generated Certificates for {$count} student(s)!";
...
// Fetch students without Certificates (with filters + pagination)
$list_sql = "
...
<head>
    ...
    <title>Bulk Generate Certificates</title>
            ...
            <div class="col-lg-11">
                <div class="card shadow-lg border-0 rounded-4">
                    <div class="card-header bg-gradient bg-primary text-white text-center py-5">
                        <h2 class="mb-0">
                            <i class="bi bi-card-text me-2"></i>Bulk Generate Certificates
                        </h2>
                        <p class="mb-0 opacity-90 mt-2">Select students → Generate Certificates in bulk → Print
                            individually</p>
                    </div>
                    ...
                    <div class="card-body p-5">
                        ...
                        <div class="row mb-5">
                            <div class="col-md-4">
                                <div class="card stats-card text-center p-4 rounded-3 h-100">
                                    ...
                                    <small>Pending Certificates (Filtered)</small>
                                </div>
                            </div>
                            <div class="col-md-4">
                        ...
                        <form method="GET" class="row g-3 mb-4">
                            <div class="col-md-4">
                                ...
                                <input type="text" name="reg_no" class="form-control"
                                       value="<?= htmlspecialchars($filter_reg_no) ?>">
                            </div>
                            <div class="col-md-4">
                                <label class="form-label">Course Code</label>
                                <input type="text" name="course" class="form-control"
                                       value="<?= htmlspecialchars($filter_course) ?>">
                            </div>
                            <div class="col-md-4">
                                <label class="form-label">Center Code</label>
                                <input type="text" name="center" class="form-control"
                                       value="<?= htmlspecialchars($filter_center) ?>">
                            </div>
                        ...
                        <!-- BULK SELECTION FORM -->
                        <h5 class="section-title">Select Students for Certificate Generation</h5>
                            ...
                            <div class="row g-4 mb-4">
                                <div class="col-12">
                                    <div class="select-zone text-center p-5 position-relative">
                                        ...
                                        <p class="text-muted mb-0">Check boxes → Generate Certificate status → Print individual
                                            cards</p>
                                        <?php if (empty($students)): ?>
                                            <div class="mt-4 p-4 bg-light rounded-3">
                                                <i class="bi bi-check-circle fs-3 text-success mb-3"></i>
                                                <h6 class="text-success">All Certificates Generated!</h6>
                                            </div>
                                        <?php endif; ?>
                                    </div>
                                </div>
...
<!-- STUDENTS LIST -->
                                        ...
                                        <tbody>
                                            <?php foreach ($students as $student): ?>
                                                ...
                                                <span
                                                    ...
                                                    <td>
                                                        <a href="../pdf/certificate?reg=<?= urlencode($student['reg_no']) ?>"
                            ...
                            <div class="text-end mt-4 p-4 bg-light rounded-3">
                                <button type="submit" class="btn btn-success btn-lg px-5 me-3" id="generateBulkBtn"
                                    ...
                                    Generate Selected Certificates (<?= count($students) ?>)
                                </button>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function updateGenerateButton() {
                ...
                generateBtn.innerHTML = `Generate Selected Certificates (${checkedCount})`;
