...
// --------- FILTERS (Reg No, Course, Center) ----------
$filter_reg_no = $_GET['reg_no'] ?? '';
$filter_course = $_GET['course'] ?? '';
$filter_center = $_GET['center'] ?? '';

$where = "WHERE (s.admit_card_gen = 'No' OR s.admit_card_gen IS NULL)";
$params = [];
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!empty($selected_reg_nos)) {
        ...
        $stmt = $pdo->prepare("UPDATE students SET admit_card_gen = 'Yes' WHERE reg_no IN ($placeholders)");
        $stmt->execute($selected_reg_nos);

        $count = count($selected_reg_nos);
        $message = "Successfully generated Admit Cards for {$count} student(s)!";
...
// Fetch students without Admit Cards (with filters and pagination)
$sql = "
...
<head>
    ...
    <title>Bulk Generate Admit Cards</title>
            ...
            <div class="col-lg-11">
                <div class="card shadow-lg border-0 rounded-4">
                    <div class="card-header bg-gradient bg-primary text-white text-center py-5">
                        <h2 class="mb-0">
                            <i class="bi bi-card-text me-2"></i>Bulk Generate Admit Cards
                        </h2>
                        <p class="mb-0 opacity-90 mt-2">Select students → Generate Admit cards in bulk → Print
                            individually</p>
                    </div>
                    ...
                    <div class="card-body p-5">
                        ...
                        <div class="row mb-4">
                            <div class="col-md-4">
                                <div class="card stats-card text-center p-4 rounded-3 h-100">
                                    ...
                                    <small>Pending Admit Cards (Filtered)</small>
                                </div>
                            </div>
                            <div class="col-md-4">
                        ...
                        <!-- BULK SELECTION FORM -->
                        <h5 class="section-title">Select Students for Admit Card Generation</h5>
                        <form method="POST" id="bulkForm">
                            <div class="row g-4 mb-4">
                                <div class="col-12">
                                    <div class="select-zone text-center p-5 position-relative">
                                        ...
                                        <p class="text-muted mb-0">Check boxes → Generate Admit Card status → Print
                                            individual
                                            cards</p>
                                        <?php if (empty($students)): ?>
                                            <div class="mt-4 p-4 bg-light rounded-3">
                                                <i class="bi bi-check-circle fs-3 text-success mb-3"></i>
                                                <h6 class="text-success">All Admit Cards Generated!</h6>
                                            </div>
                                        <?php endif; ?>
                                    </div>
                                </div>
                                        ...
                                        <tbody>
                                            <?php foreach ($students as $student): ?>
                                                <tr class="card-hover">
                                                    ...
                                                    <td>
                                                        <a href="../pdf/admitcard?reg=<?= urlencode($student['reg_no']) ?>"
                            ...
                            <div class="text-end mt-4 p-4 bg-light rounded-3">
                                <button type="submit" class="btn btn-success btn-lg px-5 me-3" id="generateBulkBtn"
                                    ...
                                    Generate Selected Admit Cards (<?= count($students) ?>)
                                </button>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function updateGenerateButton() {
                ...
                generateBtn.innerHTML = `Generate Selected Admit Cards (${checkedCount})`;
